...
用法:
  python training/train_lstm_v3.py --symbol SOL
  python training/train_lstm_v3.py --symbol BTC --epochs 150
  python training/train_lstm_v3.py --symbol ETH --load-v1
"""
...
DEFAULT_CONFIG = {
    ...
    'training': {
        'epochs': 200,
        'batch_size': 16,
        'learning_rate': 0.0005,  # v3: 平衡学习率 (v1 的 0.0005)
        'weight_decay': 0.0001,
        'lookback_window': 60,
        'forecast_horizon': 1,
        'train_split': 0.8,
...
def setup_logging(symbol: str, version: str = 'v3'):
    """設定日誌"""
    global logger
...
def backup_models(symbol: str):
    ...
    backup_v1_path = f'models/backup_v1/{symbol}_model_v1.pth'
    backup_v2_path = f'models/backup_v2/{symbol}_model_v2.pth'
    
    # 處理 v2 的備份
    if os.path.exists(backup_v1_path):
        os.makedirs('models/backup_v2', exist_ok=True)
        if os.path.exists(current_path):
            shutil.copy(current_path, backup_v2_path)
            logger.info(f"✓ v2 model backed up: {backup_v2_path}")
    
    # 處理 v1 的備份
    if os.path.exists(current_path) and not os.path.exists(backup_v1_path):
        os.makedirs('models/backup_v1', exist_ok=True)
        shutil.copy(current_path, backup_v1_path)
        logger.info(f"✓ v1 model backed up: {backup_v1_path}")


def calculate_combined_loss(y_true, y_pred, alpha=0.7):
    """計算組合損失（价格 + 方向）
    alpha: 价格損失的比重 (0.7 = 70% MAE + 30% 方向)
    """
    # 价格損失 (MSE)
    price_loss = nn.MSELoss()(y_pred, y_true)
    
    # 方向損失
    y_true_diff = y_true.diff(dim=0) if len(y_true.shape) > 1 else torch.diff(y_true)
    y_pred_diff = y_pred.diff(dim=0) if len(y_pred.shape) > 1 else torch.diff(y_pred)
    
    y_true_dir = torch.sign(y_true_diff)
    y_pred_dir = torch.sign(y_pred_diff)
    
    direction_loss = nn.MSELoss()(y_pred_dir, y_true_dir)
    
    # 組合損失
    combined_loss = alpha * price_loss + (1 - alpha) * direction_loss
    return combined_loss, price_loss, direction_loss
...
def train_epoch(model, train_loader, optimizer, device):
    ...
    for X_batch, y_batch in train_loader:
        ...
        # 使用組合損失
        combined_loss, price_loss, dir_loss = calculate_combined_loss(y_batch, outputs, alpha=0.7)
    ...
    return total_loss / len(train_loader), total_price_loss / len(train_loader), total_dir_loss / len(train_loader)
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='LSTM Training v3 - Balanced Optimization')
    ...
    # 設定日誌
    setup_logging(args.symbol, version='v3')
    
    logger.info('='*80)
    logger.info('LSTM MODEL TRAINING (V3 - Balanced Optimization)')
    logger.info('='*80)
    ...
    logger.info(f"Optimization: 70% Price Loss + 30% Direction Loss")
    ...
    for epoch in range(config['training']['epochs']):
        train_loss, train_price_loss, train_dir_loss = train_epoch(model, train_loader, optimizer, args.device)
        ...
        if (epoch + 1) % 10 == 0 or patience_counter >= config['optimization']['patience']:
            logger.info(f"Epoch {epoch+1:3d}/{config['training']['epochs']} | "
                      f"Train Loss: {train_loss:.6f} (P: {train_price_loss:.6f} D: {train_dir_loss:.6f}) | "
    ...
    with open(f'results/{args.symbol}_results_v3.json', 'w') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    logger.info(f"\nResults saved to results/{args.symbol}_results_v3.json")
