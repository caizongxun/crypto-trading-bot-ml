...
def setup_logging(symbol: str, version: str = 'v8'):
    """設定日誌"""
    global logger
...
def cleanup_old_models(symbol: str):
    ...
    if os.path.exists(current_path):
        os.makedirs('models/backup_v7', exist_ok=True)
        shutil.move(current_path, f'models/backup_v7/{symbol}_model_v7_old.pth')
...
def main():
    ...
    with open(f'results/{args.symbol}_results_v8.json', 'w') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    logger.info(f"\nResults saved to results/{args.symbol}_results_v8.json")
