...
"""
訓練結果可視化器
比較 v1、v2、v3 和 v4 的訓練結果，統計指標

用法:
  python visualize_results.py                    # 比較所有幣種 v1-v4
...
class ResultsVisualizer:
    ...
    def _load_results(self):
        ...
        print(f"✓ 載入結果: {len(self.v1_results)} v1, {len(self.v2_results)} v2, {len(self.v3_results)} v3, {len(self.v4_results)} v4")
    
    def plot_single_symbol(self, symbol: str):
        """針對單個幣種的 v1 vs v2 vs v3 vs v4 比較"""
        ...
        fig.suptitle(f'{symbol} - V1 vs V2 vs V3 vs V4 比較', fontsize=16, fontweight='bold')
        
        def prepare_data(v1, v2, v3, v4, key):
            versions, values, colors = [], [], []
            if v1:
                versions.append('V1')
                values.append(v1.get(key, 0))
                colors.append('#1f77b4')
            if v2:
                versions.append('V2')
                values.append(v2.get(key, 0))
                colors.append('#ff7f0e')
            if v3:
                versions.append('V3')
                values.append(v3.get(key, 0))
                colors.append('#2ca02c')
            if v4:
                versions.append('V4')
                values.append(v4.get(key, 0))
                colors.append('#d62728')
            return versions, values, colors
        
        # MAE 比較
        ax = axes[0, 0]
        versions, maes, colors = prepare_data(v1, v2, v3, v4, 'mae')
        ...
        # MAPE 比較
        ax = axes[0, 1]
        versions, mapes, colors = prepare_data(v1, v2, v3, v4, 'mape')
        mapes = [m * 100 if m else 0 for m in mapes]
        ...
        # 方向準確度 比較
        ax = axes[1, 0]
        versions, accs, colors = prepare_data(v1, v2, v3, v4, 'direction_accuracy')
        accs = [a * 100 if a else 0 for a in accs]
        ax.bar(versions, accs, color=colors, alpha=0.7, edgecolor='black')
        ...
        # RMSE 比較
        ax = axes[1, 1]
        versions, rmses, colors = prepare_data(v1, v2, v3, v4, 'rmse')
        ...
        plt.tight_layout()
        output_file = self.plots_dir / f'{symbol}_v1_vs_v2_vs_v3_vs_v4.png'
    ...
    def plot_all_comparison(self):
        """針對全部幣種的比較檔索"""
        all_symbols = set(self.v1_results.keys()) | set(self.v2_results.keys()) | set(self.v3_results.keys()) | set(self.v4_results.keys())
        if not all_symbols:
            print("✗ 沒有結果檔案")
            return
        ...
        fig.suptitle('所有幣種 - V1 vs V2 vs V3 vs V4 比較', fontsize=16, fontweight='bold')
        ...
        # MAPE 比較
        ax = axes[0, 1]
        v1_mapes = [self.v1_results.get(s, {}).get('mape', 0)*100 for s in all_symbols]
        v2_mapes = [self.v2_results.get(s, {}).get('mape', 0)*100 for s in all_symbols]
        v3_mapes = [self.v3_results.get(s, {}).get('mape', 0)*100 for s in all_symbols]
        v4_mapes = [self.v4_results.get(s, {}).get('mape', 0)*100 for s in all_symbols]
        
        ax.bar(x - 1.5*width, v1_mapes, width, label='V1', color='#1f77b4', alpha=0.8)
        ax.bar(x - 0.5*width, v2_mapes, width, label='V2', color='#ff7f0e', alpha=0.8)
        ax.bar(x + 0.5*width, v3_mapes, width, label='V3', color='#2ca02c', alpha=0.8)
        ax.bar(x + 1.5*width, v4_mapes, width, label='V4', color='#d62728', alpha=0.8)
        ax.set_xlabel('幣種')
        ax.set_ylabel('MAPE (%)')
        ax.set_title('MAPE 比較')
        ax.set_xticks(x)
        ax.set_xticklabels(all_symbols, rotation=45)
        ax.legend()
        ax.grid(axis='y', alpha=0.3)
        ...
        ax.bar(x - 1.5*width, v1_accs, width, label='V1', color='#1f77b4', alpha=0.8)
        ax.bar(x - 0.5*width, v2_accs, width, label='V2', color='#ff7f0e', alpha=0.8)
        ax.bar(x + 0.5*width, v3_accs, width, label='V3', color='#2ca02c', alpha=0.8)
        ax.bar(x + 1.5*width, v4_accs, width, label='V4', color='#d62728', alpha=0.8)
        ...
        # RMSE
        ax = axes[1, 1]
        v1_rmses = [self.v1_results.get(s, {}).get('rmse', 0) for s in all_symbols]
        v2_rmses = [self.v2_results.get(s, {}).get('rmse', 0) for s in all_symbols]
        v3_rmses = [self.v3_results.get(s, {}).get('rmse', 0) for s in all_symbols]
        v4_rmses = [self.v4_results.get(s, {}).get('rmse', 0) for s in all_symbols]
        
        ax.bar(x - 1.5*width, v1_rmses, width, label='V1', color='#1f77b4', alpha=0.8)
        ax.bar(x - 0.5*width, v2_rmses, width, label='V2', color='#ff7f0e', alpha=0.8)
        ax.bar(x + 0.5*width, v3_rmses, width, label='V3', color='#2ca02c', alpha=0.8)
        ax.bar(x + 1.5*width, v4_rmses, width, label='V4', color='#d62728', alpha=0.8)
        ax.set_xlabel('幣種')
        ax.set_ylabel('RMSE (USD)')
        ax.set_title('RMSE 比較')
        ax.set_xticks(x)
        ax.set_xticklabels(all_symbols, rotation=45)
        ax.legend()
        ax.grid(axis='y', alpha=0.3)
        ...
        plt.tight_layout()
        output_file = self.plots_dir / 'all_symbols_comparison_v1_v2_v3_v4.png'
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        print(f"✓ 已保存: {output_file}")
        plt.close()
    ...
    def generate_summary_table(self):
        """產生总结表格"""
        all_symbols = sorted(set(self.v1_results.keys()) | set(self.v2_results.keys()) | set(self.v3_results.keys()) | set(self.v4_results.keys()))
        ...
        for symbol in all_symbols:
            ...
            row = {
                ...
                'V4 準確': f"{v4.get('direction_accuracy', 0)*100:.2f}%",
            }
            data.append(row)
        
        df = pd.DataFrame(data)
        output_file = self.plots_dir / 'comparison_summary_v1_v2_v3_v4.csv'
    ...
    def run(self, symbol: str = None):
        ...
        print(f"  - 圖表位置: plots/")
        print(f"  - 總结表: plots/comparison_summary_v1_v2_v3_v4.csv")
